...
const setDocument = function setDocument (doc) {
  ...
  try {
    document = doc['es:Document'][0]['ds:Object'][0]['_']
  } catch (err) {
    throw err
  }
...
const setId = function setId (document) {
  let id
  try {
    id = document['es:Document'][0]['ds:Object'][0]['$']['Id']
  } catch (err) {
...
class EAktaDocument {
  constructor (doc) {
    ...
    try {
      let root = doc['es:Document'][0]
      documentProfile = root['es:DocumentProfile'][0]
      documentSignature = root['ds:Signature'] ? root['ds:Signature'][0] : undefined
    } catch (err) {
